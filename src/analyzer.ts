...
export class SvelteMigrationAnalyzer {
  ...
  async findSvelteFiles(searchPaths: string[], excludePaths: string[]): Promise<string[]> {
  ...
  async analyzeFile(filename: string, content: string): Promise<AnalysisResult> {
    ...
    if (this.anthropicService && (issues.length > 0 || warnings.length > 0)) {
      try {
        aiSuggestions = await this.anthropicService.analyzeSvelteFile(filename, content, issues, warnings)
      } catch (error) {
  ...
  private getMigrationPatterns() {
    return [
      // Svelte 4 reactive statements ($:)
      {
        ...
        regex: /^\s*\$:\s+/,
        message: 'Reactive statement ($:) should be replaced with $derived or $effect',
      ...
      // Event handlers with on: directive
      {
        rule: 'on-directive',
        regex: /\son:(\w+)=/,
        message: 'on: event directives should be replaced with event properties',
      ...
      // Component instantiation with new
      {
        ...
        suggestion: 'Use mount() or hydrate() instead of new Component()',
        severity: 'error' as const
      },
      ...
      // <svelte:component>
      {
        ...
        message: 'svelte:component is no longer necessary in Svelte 5',
        suggestion: 'You can use dynamic components directly: <Thing />',
        severity: 'warning' as const
      },
      ...
      // let: directive in slots
      {
        rule: 'let-directive',
        regex: /let:(\w+)/,
        message: 'let: directive in slots should be replaced with snippet parameters',
      ...
      // Event modifiers
      {
        ...
        suggestion: 'Handle event.preventDefault(), event.stopPropagation() etc. in the handler',
        severity: 'warning' as const
      },
