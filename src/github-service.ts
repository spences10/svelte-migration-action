...
export class GitHubService {
	...
	async getChangedSvelteFiles(): Promise<string[]> {
		...
		try {
			...
			core.info(
				`Found ${svelteFiles.length} changed Svelte files in PR`,
			);
			return svelteFiles;
		} catch (error) {
	...
	async createOrUpdateComment(summary: string): Promise<void> {
		...
		try {
			...
			if (!pullNumber) {
				core.warning(
					'No pull request number found, cannot create comment',
				);
				return;
			}
			...
			if (existingComment) {
				...
				core.info(
					'Updated existing PR comment with analysis results',
				);
			} else {
				// Create new comment
	...
	async createCheckRun(
		...
		try {
			...
			const conclusion =
				issues > 0 ? 'failure' : warnings > 0 ? 'neutral' : 'success';
			const title =
				issues > 0
		...
		try {
			...
			for (const chunk of chunks) {
				...
				await this.octokit.rest.checks.create({
					...
					head_sha: sha,
					status: 'completed',
					conclusion: issues.some((i) => i.severity === 'error')
						? 'failure'
						: 'neutral',
					output: {
