...
async function run(): Promise<void> {
  try {
    ...
    const anthropic = enableAiAnalysis && anthropicApiKey 
    ...
    if (github.context.eventName === 'pull_request' && (totalIssues > 0 || totalWarnings > 0)) {
    ...
    if (failOnWarning && totalWarnings > 0) {
      core.setFailed(`Migration analysis failed: ${totalWarnings} warnings found`)
    }
...
function generateSummary(results: any[], totalIssues: number, totalWarnings: number): string {
  let summary = '# 🔄 Svelte Migration Analysis Results\n\n'
  
  if (totalIssues === 0 && totalWarnings === 0) {
    summary += '✅ **Great news!** No Svelte 4 patterns detected in your codebase.\n\n'
  ...
  if (totalIssues > 0) {
    ...
    summary += 'These are breaking changes that must be addressed for Svelte 5:\n\n'
  ...
  if (totalWarnings > 0) {
    ...
    summary += 'These patterns will still work but are deprecated in Svelte 5:\n\n'
