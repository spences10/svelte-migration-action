...
describe("SvelteMigrationAnalyzer", () => {
  let analyzer: SvelteMigrationAnalyzer;

  beforeEach(() => {
    analyzer = new SvelteMigrationAnalyzer();
  });
  ...
  describe("analyzeFile", () => {
    it("should detect createEventDispatcher usage", async () => {
      ...
      const result = await analyzer.analyzeFile("test.svelte", content);
    ...
    it("should handle AI analysis when service is provided", async () => {
      ...
      const analyzerWithAI = new SvelteMigrationAnalyzer(mockAnthropicService);
      ...
      const result = await analyzerWithAI.analyzeFile("test.svelte", content);
      ...
      expect(mockAnthropicService.analyzeSvelteFile).toHaveBeenCalledWith(
        "test.svelte",
        content,
        [],
        expect.any(Array)
    ...
    it("should handle AI analysis errors gracefully", async () => {
      const mockAnthropicService = {
        analyzeSvelteFile: vi.fn().mockRejectedValue(new Error("API Error")),
      } as unknown as AnthropicService;
  ...
  describe("findSvelteFiles", () => {
    it("should find Svelte files in specified paths", async () => {
      ...
      const result = await analyzer.findSvelteFiles(["src"], ["node_modules"]);
    ...
    it("should analyze multiple files", async () => {
      ...
      vi.spyOn(fs, "readFile").mockImplementation((filename) => {
        ...
        if (filename === "file2.svelte") {
          return Promise.resolve("const dispatch = createEventDispatcher();");
        }
